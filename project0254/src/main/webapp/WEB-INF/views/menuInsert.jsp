...
<body>
	<form method="POST" action="menuInsert.do" enctype="multipart/form-data">
		<div class="all">
			<div class="side">
				<%@ include file="/WEB-INF/views/common/ownerSideBar.jsp"%>
			</div>
			<input type="hidden" name="menu_id" value=0> <input
				type="hidden" name=menu_store_id value="${store.store_id }">
			...
			<div class="content1">
				...
				<div class="full">
					<div class="insert">
							...
							<select name="menu_category">
								<c:forEach items="${menucategory}" var="category">
									...
									<option value='${category.menu_category_seq}'
										<c:if test="${category.menu_category_seq == nowCategoryId}">selected</c:if>>${category.menu_category_name}
									</option>
								</c:forEach>
							</select>
						</div>
						<div class="flex-container">
							...
							<!-- <input type="checkbox" name="item" value="menu_is_hot">대표메뉴1 -->
							<input id="hotCheck" type="checkbox" name="menu_is_hot">대표메뉴
							<input id="signatureCheck" type="checkbox" name="menu_is_signature">시그니처 <br>
						</div>
						...
						<h4>빵 한 줄 소개</h4>
						<textarea name="menu_describe" id="menu_describe" rows="5">${menu.menu_describe}</textarea>
						<div class="flex-container">
							<h4>가격</h4>

							<div class="price-input">
								<input id="price" type="int" oninput="makeDiscount()"
									name="menu_price" value=0 /> <span>원</span>
							</div>
						</div>
						<h4>할인할 금액</h4>
						<div class="price-input">
							<input id="discountprice" oninput="makeDiscount()" type="int"
								name=menu_discount_price value=0 /><span>원</span>
							<br />
						</div>
						...
						<h4>빵 나오는 시간</h4>
						<div class="price-input">
							<input type="int" name=menu_time value="8" /><span>시</span>
							<br />
						</div>
						<h4>나오는 빵</h4>
						<div class="price-input">
							<input type="int" name=menu_making_count
								 value="0" /><span>개</span> <br />
						</div>
					...
					<div class="preview">
						...
						<div class="divimage">
							<fieldset id="imagebox">
								<p>이미지를 선택해주세요</p>
								<input type="file" name="img_menu_path" id="0_img_input" onchange="setImgPath(event)" >
							</fieldset>
							<div id="menu_image_preview">
								<img src="resources/images/notStoreImg.png" id="upload_img">						
							</div>
						...
						<div class="divimage">
							<fieldset id="thumbimagebox">
								...
								<input type="file" name="img_menu_thumb_path" id="0_img_thumb_input" onchange="setThumbnail(event)" >
							</fieldset>
							<div id="menu_thumb_image_preview">
								<img src="resources/images/notStoreImg.png" id="upload_img_thumb">						
							</div>
				...
				<div class="w3-container w3-center">
					<button class="indoorbutton" type="submit">메뉴입력</button>
				</div>
...
/* 			console.log("textarea", window.getComputedStyle(textarea).style.width);
			input.style.width = window.getComputedStyle(textarea).style.width; */
		}
	</script>
	<script>
